...
from .models import Contato, TentativaLogin, PerfilUsuario, Moeda, PreferenciaMoeda, RequisicaoCompra, Transacao, HistoricoTransacao, TransacaoMpesa, Denuncia, Avaliacao, TermosAceitacao
from .forms import ContatoForm, PerfilUsuarioForm, RequisicaoCompraForm, CustomUserCreationForm, AvaliacaoForm
...
def registar(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            TermosAceitacao.objects.create(
                usuario=user,
                versao=TermosAceitacao.get_versao_atual(),
                aceite=True
            )
...
def detalhe_denuncia(request, denuncia_id):
    ...
    return render(request, 'core/moderacao/detalhe_denuncia.html', {'denuncia': denuncia})
...
def termos_condicoes(request):
    return render(request, 'core/termos_condicoes.html')
...
def avaliar(request, transacao_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            avaliacao.save()
            try:
                fornecedor_obj = Fornecedor.objects.get(usuario=transacao.fornecedor)
                avaliacoes = Avaliacao.objects.filter(fornecedor=transacao.fornecedor)
                total = avaliacoes.count()
                if total > 0:
                    soma = sum(a.nota for a in avaliacoes)
                    fornecedor_obj.avaliacao_media = round(soma / total, 2)
